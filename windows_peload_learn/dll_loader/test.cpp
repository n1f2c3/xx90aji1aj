// mmloader.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
main() {
...
#ifdef _DEBUG
	TCHAR szDllPath[] = _T("api-ms-win-code-debug-l1-1-0.dll");
#else
	TCHAR szDllPath[] = _T("api-ms-win-code-debug-l1-1-0.dll");
#endif
	...
	if (hMemModule) {
		...
		LPVOID lpAddr = (LPVOID)MemModuleHelper(MHM_FARPROC_GETPROC, hMemModule, (char *)"demoFunction", 0);
		if (lpAddr) {
			...
			if (pfnFunction(buf, MAX_PATH)) {
				char *p = (char *)"{f56fee02-16d1-44a3-b191-4d7535f92ca5}";
